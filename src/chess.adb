package body Chess is
    ...
    function is_valid_move(x_start : in Integer; y_start : in Integer; x_end : in Integer; y_end : in Integer; prev : in String) return Boolean is
...
--            with Pre => Cell'Pos(c) in white_range or Cell'Pos(c) in black_range;
        ...
        function valid_piece_move(x_start : in Integer; y_start : in Integer; x_end : in Integer; y_end : in Integer; prev : in String) return Boolean is
--            with Pre => get_piece_at(x_start, y_start, p) /= Empty
--                    and get_piece_at(x_start, y_start, p) /= Forbidden;

            function is_valid_pawn(delta_x : in Integer; delta_y : in Integer; x_start : in Integer; y_start : in Integer; x_end : in Integer; y_end : in Integer; prev : in String) return Boolean is
            ...
            begin
                ...
                if delta_y = 1 and delta_x = 0 and get_piece_at(x_end, y_end) = Empty then
                    return True;
                end if;

                -- Start move : straight 2
                if delta_y = 2 and delta_x = 0 and y_start = 2 and get_piece_at(x_end, y_end) = Empty then
                    return True;
                end if;

                if delta_y = 1 and abs delta_x = 1 then
                    -- Capture piece : en passant
                    if get_piece_at(x_end, y_end) = Empty then
                        return y_start = 5
                            and is_pawn(get_piece_at(x_end, 5))
                            and prev = L & '2' & ' ' & L & '4';
            ...
            begin
                ...
                for y in y_start .. y_start + delta_y loop
                    for x in x_start .. x_start + delta_x loop
                        if get_piece_at(x, y) /= Empty then
                            return False;
                        end if;
                    end loop;
                end loop;
            ...
            begin
                ...
                for i in 1 .. (abs delta_x) - 1 loop
                    if get_piece_at(x_start + i * dir_x, y_start + i * dir_y) /= Empty then
                        return False;
                    end if;
                end loop;
        ...
        begin
            case get_piece_at(x_start, y_start) is
                when Pawn_white     | Pawn_black    => return is_valid_pawn(delta_x, delta_y, x_start, y_start, x_end, y_end, prev);
...
--        Put_Line("x_start :" & Integer'Image(x_start) & ".");
        -- Out of Bounds check
        if get_piece_at(x_end, y_end) = forbidden then
            return False;
        end if;

        -- Check if start piece is valid
        if get_player(get_piece_at(x_end, y_end)) /= side then
            return False;
        end if;
        ...
        return valid_piece_move(x_start, y_start, x_end, y_end, prev);
    end is_valid_move;
    ...
    begin
        return (if side = White then Chess.game_board(x, y) else Chess.game_board(x, 9 - y));
    end get_piece_at;
